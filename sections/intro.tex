...
\label{sec:intro}
\IEEEPARstart{S}{patio-temporal} forecasting aims to predict future node states in graph-structured systems where temporal dynamics and spatial interactions evolve jointly, such as transportation networks, power grids, and industrial sensor clusters. In these applications, node-level trajectories are rarely self-driven; rather, they are shaped by a combination of historical operating conditions, future external drivers, and domain-specific feasibility constraints. As illustrated in Fig.~\ref{fig:multi_scale_processes}, complex network signals typically comprise shared macro-trends driven by exogenous forcing and high-frequency variations unique to individual nodes. Consequently, a practical forecasting model must learn continuous temporal representations, infer dynamic graph relations, and integrate future exogenous variables (ExG) without inducing data leakage, all while ensuring that its predictions remain compatible with operational regimes.
...
\label{sec:capacity}
Capacity denotes the expressive power of the underlying mathematical architecture. While deep learning models, particularly Transformers, possess immense expressive capability for sequence modeling, they are notoriously sensitive to the high-frequency stochastic noise embedded in raw spatio-temporal data (e.g., localized traffic congestions or turbulent wind gusts). Furthermore, standard temporal patching exacerbates this vulnerability by splitting continuous physical regimes across arbitrary boundaries. As highlighted by recent advancements in complexity-balanced sequence modeling, deploying uniform representational capacity across an entire time horizon is computationally inefficient and structurally rigid. The central challenge is designing an architecture equipped with sufficient capacity to capture macro-trend signals, while concurrently employing adaptive temporal allocation to filter node-specific noise.
