...
To simplify notation, let $\mathcal{X}_t \triangleq (\mathbf{X}_{t-T+1:t}, \mathbf{E}^{h}_{t-T+1:t}, \mathbf{E}^{f}_{t+1:t+H}, A, M_G)$ denote the set of input drivers available at time $t$. The forecasting model is a parameterized function $f_{\theta}$:
\begin{equation}
\hat{\mathbf{Y}}_{t+1:t+H} = f_{\theta}(\mathcal{X}_t),
...
\begin{equation}
\min_{\theta}\; \frac{1}{|\mathcal{D}|}\sum_{(\mathcal{X}, \mathbf{Y})\in\mathcal{D}}
\ell\big(f_{\theta}(\mathcal{X}),\mathbf{Y}\big) + \lambda_{\Omega}\mathcal{R}_{\Omega}\big(f_{\theta}(\mathcal{X}),\mathbf{Y}\big),
