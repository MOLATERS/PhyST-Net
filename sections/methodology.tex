...
\begin{tikzpicture}[
    ...
    cond/.style={module, fill=violet!8, draw=violet!60!black, minimum width=1.5cm},
    arrow/.style={->, line width=0.8pt, draw=black!75},
    note/.style={font=\tiny, align=center}
]
    % HT node
    \node[module, minimum width=3cm] (ht) at (1.5, 4.0) {$H_T$ (Latent Representation)};
    
    % Decomp block
    \draw[arrow] (ht.south) -- ++(0,-0.6) node[midway, right, note] {Patch-wise Decomp.};
    \node[cond] (hs) at (0, 2.5) {Seasonal $H_S$};
    \node[cond] (hr) at (3.0, 2.5) {Trend $H_R$};
    
    % Graph operations
    \node[circle, draw, thick, fill=teal!5] (gs) at (0, 1.2) {GC};
    \node[circle, draw, thick, fill=teal!5] (gr) at (3.0, 1.2) {GC};
    \draw[draw=red!70!black, dashed, line width=1pt, rounded corners] (-0.8, 0.8) rectangle (3.8, 1.6);
    \node[note, text=red!80!black, font=\bfseries] at (1.5, 0.6) {Distance-Masked Top-$K$ Interaction};
...
\begin{tikzpicture}[
    ...
    \node[module, fill=white, minimum width=1.5cm, minimum height=0.5cm] (hg) at (3.2, 3.8) {$H_G$};
    \node[draw, circle, thick, fill=white, minimum size=5.5mm, inner sep=0pt] (mul) at (3.2, 2.7) {$\otimes$};
    \node[draw, circle, thick, fill=white, minimum size=5.5mm, inner sep=0pt] (add1) at (3.2, 1.6) {$\oplus$};
    \node[draw, circle, thick, fill=white, minimum size=5.5mm, inner sep=0pt] (add2) at (3.2, 0.5) {$\oplus$};
    \node[module, fill=blue!6, draw=blue!60!black, minimum width=2.4cm, minimum height=0.6cm] (hf) at (3.2, -0.6) {\textbf{Conditioned State} $H_F$};
    ...
    \draw[arrow] (gamma.east) -- ++(1.8, 0) |- (mul.west);
    \draw[arrow] (beta.east) -- ++(0.5, 0) |- (add1.west);
    
    % Residual path
    \draw[arrow] (hg.east) -- ++(0.6, 0) |- (add2.east);
    \node[note, anchor=west] at (3.9, 2.0) {Residual Scaling\\$\times 0.1$};
\end{tikzpicture}
