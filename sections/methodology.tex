...
\begin{tikzpicture}[
    ...
    module/.style={draw, rounded corners=3pt, very thick, align=center, minimum height=0.45cm},
...
% Panel frames
\draw[panel] (0,0) rectangle (7.1,3.8);
\draw[panel] (7.4,0) rectangle (14.5,3.8);

\node[title, anchor=west] at (0.2,3.55) {(a) AGSP Detail};
\node[title, anchor=west] at (7.6,3.55) {(b) K-AMC Detail};

% --- (a) AGSP Detail ---
\begin{scope}[shift={(0.3,0.15)}]
    \node[anchor=west, font=\bfseries] at (0.0, 3.0) {Rigid Hard Patches};
    \draw[axis] (0.0, 0.7) -- (3.0, 0.7) node[right] {$t$};
    \draw[signal] plot[domain=0.15:2.85, samples=80] (\x,{1.2 + 0.2*sin(deg(4.0*\x)) + 0.08*sin(deg(15.0*\x))});
    
    \draw[blue!70!black, hard] (0.50, 0.7) rectangle (1.15, 2.0);
    \draw[orange!85!black, hard] (1.15, 0.7) rectangle (1.85, 2.0);
    \draw[green!60!black, hard] (1.85, 0.7) rectangle (2.50, 2.0);
    
    \node[note, text=red!75!black, align=center, font=\bfseries\tiny] (warntext) at (1.5, 2.6) {Boundary Artifacts};
    \draw[warn] (warntext.south) -- (1.15, 2.05);
    \draw[warn] (warntext.south) -- (1.85, 2.05);
\end{scope}

\begin{scope}[shift={(3.8,0.15)}]
    \node[anchor=west, font=\bfseries] at (0.0, 3.0) {Soft Overlapping Gaussians};
    \draw[axis] (0.0, 0.7) -- (3.0, 0.7) node[right] {$t$};
    \draw[signal] plot[domain=0.15:2.85, samples=80] (\x,{1.2 + 0.2*sin(deg(4.0*\x)) + 0.08*sin(deg(15.0*\x))});

    \path[fill=blue!20, draw=blue!70!black, soft]
        (0.15, 0.7) plot[domain=0.15:1.45, samples=70] (\x,{0.7 + 1.2*exp(-((\x-0.80)^2)/0.11)}) -- (1.45, 0.7) -- cycle;
    \path[fill=orange!22, draw=orange!85!black, soft]
        (0.85, 0.7) plot[domain=0.85:2.05, samples=70] (\x,{0.7 + 1.1*exp(-((\x-1.45)^2)/0.09)}) -- (2.05, 0.7) -- cycle;
    \path[fill=green!18, draw=green!60!black, soft]
        (1.45, 0.7) plot[domain=1.45:2.85, samples=70] (\x,{0.7 + 1.3*exp(-((\x-2.15)^2)/0.12)}) -- (2.85, 0.7) -- cycle;
\end{scope}

% --- (b) K-AMC Detail ---
\begin{scope}[shift={(7.5,0.25)}]
    % Left Column (KAN)
    \node[input, text width=1.4cm] (ef) at (1.1, 3.0) {Future ExG $\mathbf{E}^f$};
    \draw[arrow] (ef.south) -- ++(0,-0.25) node[midway, right, note] {Align};

    \node[kan, minimum height=1.2cm, minimum width=2.0cm] (kanbox) at (1.1, 1.7) {};
    \node[anchor=north, font=\bfseries\tiny] at (1.1, 2.15) {KAN Block};
    \fill (0.6, 1.8) circle (1.2pt);
    \fill (1.6, 1.8) circle (1.2pt);
    \fill (1.1, 1.25) circle (1.2pt);
    \draw[violet!80!black, ->, line width=1pt] (0.6, 1.8) .. controls (0.5, 1.5) and (0.8, 1.35) .. (1.1, 1.25);
    \draw[violet!80!black, ->, line width=1pt] (1.6, 1.8) .. controls (1.7, 1.5) and (1.4, 1.35) .. (1.1, 1.25);
    \node[violet!80!black, note] at (0.5, 1.3) {$\phi(x)$};

    \node[cond, text width=0.6cm] (gamma) at (0.55, 0.45) {$\gamma$};
    \node[cond, text width=0.6cm] (beta) at (1.65, 0.45) {$\beta$};
    \draw[line width=0.8pt, draw=black!75] (1.1, 1.05) -- (1.1, 0.8);
    \draw[arrow] (1.1, 0.8) -| (gamma.north);
    \draw[arrow] (1.1, 0.8) -| (beta.north);

    % Right Column (Modulation)
    \node[draw, rounded corners=2pt, very thick, fill=white, minimum height=0.45cm, minimum width=1.5cm] (hg) at (4.5, 3.0) {$H_G$};
    \node[draw, circle, thick, fill=white, minimum size=4.5mm, inner sep=0pt] (mul) at (4.5, 2.2) {$\otimes$};
    \node[draw, circle, thick, fill=white, minimum size=4.5mm, inner sep=0pt] (add1) at (4.5, 1.4) {$\oplus$};
    \node[draw, circle, thick, fill=white, minimum size=4.5mm, inner sep=0pt] (add2) at (4.5, 0.6) {$\oplus$};
    \node[draw, rounded corners=2pt, very thick, fill=blue!6, draw=blue!60!black, minimum width=2.0cm, minimum height=0.5cm] (hf) at (4.5, -0.2) {\textbf{Conditioned State} $H_F$};
    ...
    % Cross-column paths (Now cleaner)
    \draw[arrow] (gamma.east) -- ++(0.4, 0) |- (mul.west);
    \draw[arrow] (beta.east) -- ++(0.3, 0) |- (add1.west);
    
    % Tight Residual path
    \draw[arrow] (hg.east) -- ++(0.3, 0) |- (add2.east);
    \node[note, align=left] at (5.0, 1.4) {Res. $\times 0.1$};
\end{scope}
\end{tikzpicture}
