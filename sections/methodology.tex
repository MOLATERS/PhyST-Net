...
\begin{tikzpicture}[
    ...
    \begin{scope}[shift={(0, 2.5)}]
        ...
        \node[text=red!80!black, font=\bfseries\scriptsize, align=center] (art) at (1.6, 1.7) {Temporal\\Discontinuity};
...
\begin{align}
H_{tr} &= \operatorname{GELU}(\operatorname{Conv1d}(H_{in})), \label{eq:rstmf_decomp_s} \\
H_{se} &= H_{in} - H_{tr}. \label{eq:rstmf_decomp_r}
\end{align}
...
\begin{tikzpicture}[
    ...
    >=Stealth,
    x=1.3cm, y=1.3cm,
    module/.style={draw=black!80, rounded corners=3pt, very thick, align=center, minimum height=0.6cm, fill=white},
    cond/.style={module, fill=violet!5, draw=violet!70!black, minimum width=2.0cm},
    arrow/.style={->, line width=0.9pt, draw=black!75},
    note/.style={font=\tiny, align=center}
]
    % Background Boxes
    \draw[fill=blue!3, draw=none, rounded corners] (-0.5, 3.6) rectangle (7.5, 5.0);
    \node[anchor=north west, font=\bfseries, text=blue!80!black] at (-0.5, 5.0) {1. Latent Patch Decomposition};

    \draw[fill=red!3, draw=none, rounded corners] (-0.5, 1.2) rectangle (7.5, 3.4);
    \node[anchor=north west, font=\bfseries, text=red!80!black] at (-0.5, 3.4) {2. Branch-wise Distance-Masked Top-$K$ Graph Interaction};

    \draw[fill=green!3, draw=none, rounded corners] (-0.5, -1.6) rectangle (7.5, 1.0);
    \node[anchor=north west, font=\bfseries, text=green!80!black] at (-0.5, 1.0) {3. Learnable Gated Fusion};

    % 1. Decomp
    \node[module, minimum width=3.5cm] (hin) at (3.5, 4.4) {$H_{in}$ (Latent Representation)};
    \node[cond] (htr) at (1.5, 3.8) {\textbf{Trend} $H_{tr}$};
    \node[cond] (hse) at (5.5, 3.8) {\textbf{Seasonal} $H_{se}$};
    \draw[arrow] (hin.south) -- (htr.north) node[midway, left, align=right] {Conv1D};
    \draw[arrow] (hin.south) -- (hse.north) node[midway, right, align=left] {$-$\\ (Residual)};

    % 2. Graph Ops
    \node[module, align=center] (mask) at (3.5, 2.6) {Distance Matrix $D$ \\ $\odot$ Top-$K$ Mask $M_K$};
    \node[circle, draw, thick, fill=teal!10, minimum size=0.8cm] (gctr) at (1.5, 1.6) {GCN};
    \node[circle, draw, thick, fill=teal!10, minimum size=0.8cm] (gcse) at (5.5, 1.6) {GCN};
    \draw[->, thick, dashed, draw=red!70!black] (mask) -- (gctr);
    \draw[->, thick, dashed, draw=red!70!black] (mask) -- (gcse);
    \draw[arrow] (htr.south) -- (gctr.north);
    \draw[arrow] (hse.south) -- (gcse.north);

    % 3. Fusion
    \node[module, minimum width=2cm] (gate) at (3.5, 0.4) {Gate $\sigma(\cdot)$};
    \node[draw, circle, thick, fill=white, inner sep=1pt] (mul1) at (1.5, 0.4) {\Large $\otimes$};
    \node[draw, circle, thick, fill=white, inner sep=1pt] (mul2) at (5.5, 0.4) {\Large $\otimes$};
    \node[draw, circle, thick, fill=white, inner sep=1pt] (add) at (3.5, -0.4) {\Large $\oplus$};
    \node[module, fill=blue!5, draw=blue!70!black, minimum width=3.5cm] (hg) at (3.5, -1.2) {\textbf{Graph-Enhanced State} $H_G$};

    \draw[arrow] (gctr) -- (mul1);
    \draw[arrow] (gcse) -- (mul2);
    \draw[arrow] (gate) -- (mul1) node[midway, above] {$g$};
    \draw[arrow] (gate) -- (mul2) node[midway, above] {$1-g$};
    \draw[arrow] (mul1) |- (add);
    \draw[arrow] (mul2) |- (add);
    \draw[arrow] (add) -- (hg);
\end{tikzpicture}
\caption{Detailed architecture of R-STMF. The input latent representation $H_{in}$ undergoes manifold decomposition. The seasonal (residual) and trend (Conv1D) components independently undergo branch-wise distance-masked graph convolution prior to gated fusion.}
...
\begin{tikzpicture}[
    ...
    \draw[arrow] (ef.south) -- ++(0,-0.5) node[midway, right, align=left] {AvgPool1d\\Temporal align};
    ...
    \node[module, fill=yellow!10, draw=orange!80!black, minimum height=0.5cm] (scale) at (3.5, 0.5) {$\times\ 0.1$};
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (add2) at (3.5, -0.4) {\Large $\oplus$};
    \node[module, fill=blue!8, draw=blue!70!black, minimum width=2.8cm, minimum height=0.7cm] (hf) at (3.5, -1.3) {\textbf{Conditioned State} $H_F$};

    \draw[arrow] (hg.south) -- (mul.north);
    \draw[arrow] (mul.south) -- (add1.north);
    \draw[arrow] (add1.south) -- (scale.north);
    \draw[arrow] (scale.south) -- (add2.north);
    ...
    % Residual path
    \draw[arrow, draw=gray!80, line width=1pt] (hg.east) -- ++(0.8, 0) |- (add2.east);
    
    % Formula
    \node[font=\bfseries] at (1.75, -2.2) {$H_F = H_G + 0.1 \cdot (\gamma \odot H_G + \beta)$};
\end{tikzpicture}
\caption{Mechanics of K-AMC. Future exogenous drivers are aligned and processed through spline-based KAN edges to produce dynamic modulation parameters. The modulation updates the graph state via a scaled residual connection.}
