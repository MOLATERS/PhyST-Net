...
% Fixed bounding box
\path[use as bounding box] (-0.55,-3.75) rectangle (5.85,5.55);

% =========================================================
% Background blocks
% =========================================================
\begin{pgfonlayer}{background}
    \draw[fill=blue!3, draw=blue!20, rounded corners]
        (-0.35,3.35) rectangle (5.55,5.40);
    \node[title, text=blue!80!black] at (-0.25,5.32)
        {1. Latent Patch Decomposition};

    \draw[fill=orange!3, draw=orange!20, rounded corners]
        (-0.35,-0.65) rectangle (5.55,2.65);
    \node[title, text=orange!85!black] at (-0.25,2.55)
        {2. Branch-wise Distance-Masked Graph Interaction};

    \draw[fill=green!3, draw=green!20, rounded corners]
        (-0.35,-3.50) rectangle (5.55,-1.15);
    \node[title, text=green!80!black] at (-0.25,-1.25)
...
% 1. Latent Patch Decomposition
% =========================================================
\node[module, fill=gray!5, minimum width=3.35cm, minimum height=0.50cm]
    (hin) at (2.55,4.75)
    {\textbf{$H_{in}$} (Latent Representation)};

\node[module, minimum width=1.35cm, minimum height=0.55cm]
    (conv) at (1.05,3.78)
    {Conv1D\\[-1pt] + GELU};

\node[op] (sub) at (4.05,3.78) {\Large $-$};

\node[lab, anchor=west] at (4.36,3.78)
    {$H_{se}=H_{in}-H_{tr}$};

% Decomposition arrows
\draw[arrow] (hin.south) -- (1.05,4.35) -- (conv.north);
\draw[arrow] (hin.south) -- (4.05,4.35) -- (sub.north);
\draw[arrow] (conv.east) -- (sub.west);

\node[cond] (htr) at (1.05,2.10) {\textbf{Trend} $H_{tr}$};
\node[cond] (hse) at (4.05,2.10) {\textbf{Seasonal} $H_{se}$};

\draw[arrow] (conv.south) -- (htr.north);
\draw[arrow] (sub.south) -- (hse.north);

% =========================================================
% 2. Branch-wise masked graph interaction
% =========================================================
\node[module,
    fill=red!5,
    draw=red!75!black,
...
{\scriptsize Distance Mask $\mathcal{M}_{dist}$\\[-1pt]
\scriptsize branch-specific Top-$K$};

\node[prop] (gctr) at (1.05,0.05)
{Masked Top-$K$\\[-1pt] Graph Prop.\\[-1pt] $\mathbf{A}_{tr}$};

\node[prop] (gcse) at (4.05,0.05)
{Masked Top-$K$\\[-1pt] Graph Prop.\\[-1pt] $\mathbf{A}_{se}$};

% Main branch arrows: clean vertical
\draw[arrow] (htr.south) -- (gctr.north);
\draw[arrow] (hse.south) -- (gcse.north);

% Mask arrows: curved, not overlapping with main data arrows
\draw[maskarrow]
    (mask.south west) .. controls (1.75,0.82) and (1.35,0.72) .. (gctr.north east);

\draw[maskarrow]
    (mask.south east) .. controls (3.35,0.82) and (3.75,0.72) .. (gcse.north west);

% Branch graph outputs
\node[lab, font=\bfseries\scriptsize] (hgtr) at (1.05,-0.78) {$H_{tr}^{G}$};
\node[lab, font=\bfseries\scriptsize] (hgse) at (4.05,-0.78) {$H_{se}^{G}$};
...
% 3. Learnable gated fusion
% =========================================================
\node[module, minimum width=1.95cm, minimum height=0.50cm]
    (gate) at (2.55,-1.82)
    {Gate $\sigma(\cdot)$};

\node[op] (multr) at (1.05,-1.82) {\Large $\otimes$};
\node[op] (mulse) at (4.05,-1.82) {\Large $\otimes$};

\node[op] (add) at (2.55,-2.62) {\Large $\oplus$};
...
{\textbf{Graph-Enhanced State} $H_G$};

% Fusion arrows
\draw[arrow] (hgtr.south) -- (multr.north);
\draw[arrow] (hgse.south) -- (mulse.north);

\draw[arrow] (gate.west) -- (multr.east)
    node[midway, above, lab] {$1-g$};

\draw[arrow] (gate.east) -- (mulse.west)
    node[midway, above, lab] {$g$};

\draw[arrow] (multr.south) |- (add.west);
\draw[arrow] (mulse.south) |- (add.east);
\draw[arrow] (add.south) -- (hg.north);

\end{tikzpicture}
\caption{Detailed architecture of R-STMF. The input latent representation $H_{in}$ is decomposed into trend and seasonal manifolds. The two branches independently perform distance-masked Top-$K$ graph propagation and are then recombined through learnable gated fusion.}
