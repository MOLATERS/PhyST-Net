...
\begin{tikzpicture}[
    ...
    \begin{scope}[shift={(0, 2.5)}]
        ...
        \draw[blue!70!black, hard, fill=blue!20] (0.4, 0) rectangle (1.2, 1.2);
        \draw[orange!85!black, hard, fill=orange!20] (1.2, 0) rectangle (2.0, 1.2);
        \draw[green!60!black, hard, fill=green!20] (2.0, 0) rectangle (2.8, 1.2);
        
        \node[text=red!80!black, font=\bfseries\scriptsize, align=center] (art1) at (1.2, -0.45) {Temporal Discontinuity};
        \draw[warn] (art1.north) -- (1.2, 0.5);
        \draw[warn] (art1.north) -- (2.0, 0.5);
    \end{scope}
    ...
    \begin{scope}[shift={(0, -0.2)}]
        ...
        \path[fill=blue!30, draw=blue!80!black, soft] (0.2,0) plot[domain=0.2:1.8, samples=50] (\x,{1.1*exp(-((\x-1)^2)/0.15)}) -- (1.8,0) -- cycle;
        \node[blue!80!black, font=\scriptsize] at (1.0, 0.45) {$\mu_1, \sigma_1$};
        
        \path[fill=orange!30, draw=orange!90!black, soft] (0.8,0) plot[domain=0.8:2.4, samples=50] (\x,{0.9*exp(-((\x-1.6)^2)/0.1)}) -- (2.4,0) -- cycle;
        \node[orange!90!black, font=\scriptsize] at (1.6, 0.35) {$\mu_2, \sigma_2$};
        
        \path[fill=green!30, draw=green!60!black, soft] (1.4,0) plot[domain=1.4:3.0, samples=50] (\x,{1.2*exp(-((\x-2.2)^2)/0.2)}) -- (3.0,0) -- cycle;
        \node[green!60!black, font=\scriptsize] at (2.2, 0.5) {$\mu_3, \sigma_3$};
        
        \node[anchor=north west, font=\tiny\itshape, text=black!80] at (0.2, -0.2) {* Learnable center ($\mu_i$) and bandwidth ($\sigma_i$). Normalized overlapping weights: $\sum_i \tilde{W}_i(\tau) = 1$.};
    \end{scope}
\end{tikzpicture}
\caption{Mechanics of AGSP. By replacing discrete sequence chunking with continuous, differentiable density windows, the model autonomously mitigates boundary artifacts and preserves temporal continuity.}
...
\begin{tikzpicture}[
    ...
    prop/.style={module, fill=teal!5, draw=teal!70!black, minimum width=1.8cm, minimum height=0.6cm},
    arrow/.style={->, line width=1pt, draw=black!75},
    note/.style={font=\tiny, align=center}
]
    % Background Boxes
    \begin{pgfonlayer}{background}
        \draw[fill=blue!3, draw=none, rounded corners] (-0.3, 3.4) rectangle (5.3, 5.4);
        \node[anchor=north west, font=\bfseries\scriptsize, text=blue!80!black] at (-0.3, 5.4) {1. Latent Patch Decomposition};

        \draw[fill=red!3, draw=none, rounded corners] (-0.3, -0.6) rectangle (5.3, 2.6);
        \node[anchor=north west, font=\bfseries\scriptsize, text=red!80!black] at (-0.3, 2.6) {2. Branch-wise Masked Graph Interaction};

        \draw[fill=green!3, draw=none, rounded corners] (-0.3, -3.4) rectangle (5.3, -1.2);
    ...
    \draw[arrow] (hin.south) -- (4.0, 4.4) -- (sub.north) node[midway, right, note] {Residual};
    ...
    % 2. Graph Ops
    \node[module, fill=red!5, draw=red!70!black, dashed, minimum width=4cm, minimum height=0.5cm] (mask) at (2.5, 1.0) {Distance Prior $D \to$ Feasible Mask $\mathcal{M}_{dist}$};
    \node[prop] (gctr) at (1.0, 0.0) {Masked Top-$K$\\Graph Prop.};
    \node[prop] (gcse) at (4.0, 0.0) {Masked Top-$K$\\Graph Prop.};

    \draw[arrow, dashed, draw=red!70!black] (mask.south) -- (1.0, 0.6) -- (gctr.north);
    \draw[arrow, dashed, draw=red!70!black] (mask.south) -- (4.0, 0.6) -- (gcse.north);
...
\begin{tikzpicture}[
    ...
    kan/.style={module, fill=teal!5, draw=teal!70!black, minimum width=2.4cm, minimum height=1.6cm},
    ...
    \node[module, fill=gray!5, minimum width=1.8cm, minimum height=0.5cm] (align) at (0, 3.4) {Alignment $\Gamma$};
    \draw[arrow] (ef.south) -- (align.north) node[midway, right] {AvgPool1d};
    
    \node[kan] (kanbox) at (0, 1.7) {};
    \node[anchor=north, font=\bfseries] at (0, 2.45) {KAN Block};
    \node[anchor=north, font=\tiny\itshape, text=black!70] at (0, 2.2) {Spline Edge Functions};
    
    % Bipartite Graph inside KAN
    \fill (-0.5, 1.8) circle (1.5pt); \fill (0.5, 1.8) circle (1.5pt);
    \fill (-0.3, 1.1) circle (1.5pt); \fill (0.3, 1.1) circle (1.5pt);
    \draw[violet!80!black, ->, line width=1.1pt] (-0.5, 1.8) .. controls (-0.6, 1.4) and (-0.5, 1.3) .. (-0.3, 1.1) node[midway, left] {\tiny $\phi(x)$};
    \draw[violet!80!black, ->, line width=1.1pt] (-0.5, 1.8) -- (0.3, 1.1);
    \draw[violet!80!black, ->, line width=1.1pt] (0.5, 1.8) -- (-0.3, 1.1);
    \draw[violet!80!black, ->, line width=1.1pt] (0.5, 1.8) .. controls (0.6, 1.4) and (0.5, 1.3) .. (0.3, 1.1);
    
    \draw[arrow] (align.south) -- (0, 2.5);

    \node[cond] (gamma) at (-0.6, -0.2) {\Large $\gamma$};
    \node[cond] (beta) at (0.6, -0.2) {\Large $\beta$};
    \draw[line width=1.2pt, draw=black!75] (0, 0.9) -- (0, 0.4);
    \draw[arrow] (0, 0.4) -| (gamma.north);
    \draw[arrow] (0, 0.4) -| (beta.north);

    % Right side: Main modulation flow
    \node[input, fill=gray!10, draw=gray!80!black] (hg) at (3.2, 4.4) {\textbf{$H_G$}};
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (mul) at (3.2, 3.2) {\Large $\otimes$};
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (add1) at (3.2, 2.0) {\Large $\oplus$};
    \node[module, fill=yellow!10, draw=orange!80!black, minimum width=1.6cm, minimum height=0.5cm] (hmod) at (3.2, 0.8) {\textbf{$H_{\text{mod}}$}};

    % Convex Blending Area
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (mul_alpha) at (3.2, -0.2) {\Large $\otimes$};
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (mul_1alpha) at (4.7, -0.2) {\Large $\otimes$};
    \node[draw, circle, thick, fill=white, minimum size=6.5mm, inner sep=0pt] (add2) at (3.2, -1.2) {\Large $\oplus$};
    
    \node[input] (hf) at (3.2, -2.4) {\textbf{ExG-conditioned State} $H_F$};

    % Main vertical path
    \draw[arrow] (hg.south) -- (mul.north);
    \draw[arrow] (mul.south) -- (add1.north);
    \draw[arrow] (add1.south) -- (hmod.north);
    \draw[arrow] (hmod.south) -- (mul_alpha.north);
    \draw[arrow] (mul_alpha.south) -- (add2.north);
    \draw[arrow] (add2.south) -- (hf.north);

    % Residual Path
    \draw[arrow, draw=gray!80, line width=1.2pt] (hg.east) -| (mul_1alpha.north);
    \draw[arrow] (mul_1alpha.south) |- (add2.east);

    % Weights for Blending
    \node[anchor=east, font=\scriptsize, text=orange!80!black] at (3.1, -0.2) {$\alpha=0.1$};
    \node[anchor=west, font=\scriptsize, text=gray!80!black] at (4.8, -0.2) {$1-\alpha$};

    % Routing parameters to operators
    \draw[arrow] (gamma.east) -- ++(2.2, 0) |- (mul.west) node[pos=0.8, above, font=\scriptsize] {Scale};
    \draw[arrow] (beta.east) -- ++(1.0, 0) |- (add1.west) node[pos=0.8, above, font=\scriptsize] {Shift};
    
    % Formula Box
    \node[module, fill=white, draw=black!30, dashed, align=center] at (1.6, -3.6) {
        \textbf{Convex Blending Formula:}\\
        $H_{\text{mod}} = \gamma \odot H_G + \beta$ \\
        $H_F = (1-\alpha)H_G + \alpha H_{\text{mod}}$
    };
\end{tikzpicture}
