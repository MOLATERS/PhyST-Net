...
The token tensor $Z$ is concatenated with a learnable global token $z_{global} \in \mathbb{R}^{B \times N \times 1 \times d}$. This global token acts as a sequence-level memory bottleneck, aggregating macro-trend representations across all temporal windows to facilitate efficient long-range context fusion. The augmented tensor $[z_{global} || Z]$ is then passed to a Transformer encoder to learn temporal dependencies:
\begin{equation}
H_T=\operatorname{STEnc}([z_{global} || Z]), \quad H_T\in\mathbb{R}^{B\times N\times (P+1)\times d}.
\label{eq:temporal_encoder}
\end{equation}
After temporal encoding, the updated global token is stripped from the sequence, ensuring the output shape remains compatible with subsequent graph adapters while retaining the enriched macro-context within the temporal patches.
