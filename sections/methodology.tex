...
\label{sec:rstmf}
R-STMF serves as the graph adapter. In physical systems, spatial dependence is multi-modal: nodes interact through shared environmental trends (macro) and localized proximities (micro). By utilizing the explicitly decoupled manifolds $R$ (global token) and $U$ (patch tokens), R-STMF avoids conflating these mechanisms.
...
To enforce spatial locality and reduce computation, R-STMF employs a Top-$K$ pruning strategy. The attention matrix $A_c$ is computed by masking out elements that are not among the $K_c$ largest scores:
\begin{equation}
...
\label{eq:rstmf_topk}
\end{equation}
After spatial propagation, the global and patch tokens are fused to form the graph-enhanced latent state $H_G$.
