...
- **源码位置**：[experiments.tex L70](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex#L70)
- **待办事项**：
...
- **源码位置**：[experiments.tex L115](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex#L115)
- **待办事项**：
...
## 3. 替换正文中的数值与比例占位符
- **源码位置**：[experiments.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex) 和 [discussion.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/discussion.tex)
- **待办事项**：
  - 在正文中全局搜索 `TBD` 关键字，根据实测结果更新文本描述占位符。
  - 特别注意替换以下定量描述：
    - [experiments.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex#L54)：我们在三数据集上实现的 state-of-the-art 合规率具体百分比。
    - [experiments.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex#L88)：K-AMC 样条边计算相比传统 MLP 增加的参数和耗时比例 `\textbf{TBD}\%`，以及 PhyST-Net 单步推理的平均延迟 `\textbf{TBD}` 毫秒。
    - [experiments.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/experiments.tex#L94-L98)：消融配置移除时，三大指标下降的具体实测比例（如：RMSE 增加 `\textbf{TBD}\%`）。

## 4. 绘制四大极端气象案例研究图并注入源码
- **图像存储目录**：`figures/`（建议保存为无损 `.pdf` 或高清晰度 `.png` 格式，如 `case_a.pdf`, `case_b.pdf`, `case_c.pdf`, `case_d.pdf`）
...
- **源码替换**：
  - 打开 [case_studies.tex](file:///C:/Users/Molat/OneDrive/桌面/PhyST-Net_Paper/source/case_studies.tex#L7-L10)。
  - 将 4 联子图环境中的黑框占位代码 `\rule{0.23\textwidth}{0.18\textwidth}` 依次替换为实际引入图像代码：
...
## 5. 编译与终稿审查
- **待办事项**：
  - 注入完毕后，在本地终端执行两次 `pdflatex main.tex`（无需执行 bibtex）。
